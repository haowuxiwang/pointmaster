...
describe('uniformPlacement', () => {
  ...
  it('places a grid point exactly at anchor position (at-*)', () => {
    const anchors = [{ position: { x: 100, y: 100, z: 400 }, label: 'D1', type: 'drain-port' }]
    const points = uniformPlacement(testChamber, 12, { includeCenter: true, anchorPoints: anchors })
  ...
  it('shaker room: 12 points across 3 layers, 4 per layer', () => {
    ...
    const layerZs = Object.keys(layerGroups)
      .map(Number)
      .sort((a, b) => a - b)
...
describe('keypointsPlacement', () => {
  it('generates 8 corners + center by default', () => {
    expect(
      keypointsPlacement(testChamber, { includeCenter: true, includeFaceCenters: false }),
    ).toHaveLength(9)
  })
  it('generates 8 corners + center + 6 face centers', () => {
    expect(
      keypointsPlacement(testChamber, { includeCenter: true, includeFaceCenters: true }),
    ).toHaveLength(15)
  })
})

describe('uniformPlacement cylinder', () => {
  const cylinderChamber = {
    type: 'cylinder' as const,
    name: 'cylinder-test',
    dimensions: { width: 1000, depth: 1000, height: 800 },
    radius: 500,
  }

  it('generates points inside cylinder radius', () => {
    const points = uniformPlacement(cylinderChamber, 8)
    const cx = 500, cy = 500, r = 500
    for (const p of points) {
      const dist = Math.sqrt((p.position.x - cx) ** 2 + (p.position.y - cy) ** 2)
      expect(dist).toBeLessThanOrEqual(r + 1) // small tolerance
    }
  })

  it('generates correct count for cylinder', () => {
    const points = uniformPlacement(cylinderChamber, 12)
    // Cylinder has 4 corners (not 8), so minimum is lower
    expect(points.length).toBeGreaterThanOrEqual(4)
  })
})

describe('uniformPlacement multi-layer', () => {
  it('distributes points across layers', () => {
    const multiLayerChamber = {
      type: 'cuboid' as const,
      name: 'multi',
      dimensions: { width: 1000, depth: 600, height: 900, layers: 3 },
    }
    const points = uniformPlacement(multiLayerChamber, 12)
    const zValues = new Set(points.map((p) => Math.round(p.position.z)))
    expect(zValues.size).toBeGreaterThanOrEqual(2)
  })
})

describe('uniformPlacement with anchor points', () => {
  it('places points near anchor positions', () => {
    const anchorPoints = [
      { position: { x: 100, y: 100, z: 0 }, label: 'drain', type: 'drain-port' },
    ]
    const points = uniformPlacement(testChamber, 10, { anchorPoints })
    expect(points.length).toBeGreaterThanOrEqual(8)
    // One point should be at the anchor position
    const nearAnchor = points.some(
      (p) => p.position.x === 100 && p.position.y === 100 && p.position.z === 0,
    )
    expect(nearAnchor).toBe(true)
  })
})

describe('uniformPlacement edge cases', () => {
  it('handles count less than corners', () => {
    const points = uniformPlacement(testChamber, 4)
    expect(points.length).toBeGreaterThanOrEqual(4)
  })
  ...
  it('no center when includeCenter is false', () => {
    const points = uniformPlacement(testChamber, 12, { includeCenter: false })
    // Center should not be exactly at chamber center
    const hasCenter = points.some(
      (p) =>
        p.position.x === testChamber.dimensions.width / 2 &&
        p.position.y === testChamber.dimensions.height / 2,
    )
    // This is not a strict test since corners can overlap, but center shouldn't be forced
    expect(points.length).toBeGreaterThanOrEqual(8)

...
interface CylinderParams { radius: number; height: number; }
...
export function getCylinderVertices(params: CylinderParams, segments: number = 16): Point3D[] {
  ...
  for (let i = 0; i < segments; i++) {
    ...
    vertices.push({ x: radius + radius * Math.cos(angle), y: radius + radius * Math.sin(angle), z: 0 });
  }
  for (let i = 0; i < segments; i++) {
    const angle = (2 * Math.PI * i) / segments;
    vertices.push({ x: radius + radius * Math.cos(angle), y: radius + radius * Math.sin(angle), z: height });
  }
  return vertices;
}

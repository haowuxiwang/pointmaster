...
export function cylinderPath(
  ...
  const backArc = (cx: number, cy: number) => `M ${cx - r} ${cy} A ${r} ${ry} 0 0 0 ${cx + r} ${cy}`
  ...
  const sideLines = [`M ${-r} ${botY} L ${-r} ${topY}`, `M ${r} ${botY} L ${r} ${topY}`].join(' ')
